...
")
IP=$(curl -H "Metadata-Flavor: Google" "http://metadata.google.internal/computeMetadata/v1/instance/networ
k-interfaces/0/ip")
CONNECTION_NAME=$(gcloud secrets versions access latest --secret="connection-name" --format='get(payload.d
ata)' | tr '_-' '/+' | base64 -d)
